// POST /imageedit-product-gen
// Body: { image_url: string, prompt: string, aspect_ratio?: string, model?: string }
// Gera shots de produto a partir de uma foto base.
...
Deno.serve(async (req) => {
  ...
  const finalPrompt =
    `Professional product photography. ${s.text}. Preserve the product's exact shape, color, ` +
    `materials, logos and proportions from the reference image. Studio lighting, sharp focus, commercial quality.`;

  const requested = (body?.model || defaultModelForTool("product-gen")).toString();
  const allowed = TOOL_MODEL_WHITELIST["product-gen"];
  const modelId = allowed.includes(requested) ? requested : defaultModelForTool("product-gen");
