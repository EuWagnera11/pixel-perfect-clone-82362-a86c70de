-- Stats agregadas pra dashboard admin
...
BEGIN
  ...
  sub_counts AS (
    SELECT
      ...
      COALESCE(SUM(
        CASE
          WHEN s.billing_cycle = 'yearly' THEN COALESCE(p.price_yearly_brl, 0) / 12.0
          ELSE COALESCE(p.price_monthly_brl, 0)
        END
      ), 0) AS mrr_brl
  ...
  recent_errors AS (
    SELECT
      g.id, g.user_id, g.tool, g.model, g.error_message, g.created_at
  ...
  SELECT jsonb_build_object(
    ...
    'generations_by_status', COALESCE((
      SELECT jsonb_object_agg(status, n) FROM gen_status
    ), '{}'::jsonb),
    'top_consumers', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
...
-- Lista de usuários mais recentes (admin only)
CREATE OR REPLACE FUNCTION public.admin_recent_users(p_limit integer DEFAULT 50)
RETURNS TABLE (
  id uuid,
  full_name text,
  tier text,
